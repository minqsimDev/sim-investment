...
## 8. 도메인 → Elastic IP (DNS A 레코드)
- Route53 사용 시: Hosted zone 생성($0.50/월) → **A 레코드** `invest.example.com` → **Elastic IP**
- 외부 등록기관 DNS 사용 시(Route53 비용 0): 등록기관에서 동일하게 **A 레코드 → Elastic IP**
- `.env`의 `DOMAIN`과 이 레코드의 호스트명이 **일치**해야 Caddy가 인증서를 발급한다.
...
## 11. 업데이트 / 롤백
```bash
cd ~/sim-investment && git pull
cd deploy && docker compose up -d --build      # 무중단에 가깝게 재기동
# 롤백: git checkout <이전커밋> 후 동일 명령
```
...
- **HTTPS 발급 실패**: A레코드가 EIP를 가리키는지, 보안그룹 80/443 열렸는지, `.env` DOMAIN 일치 확인. `docker compose logs caddy`.
