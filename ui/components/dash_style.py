...
_MOBILE_TYPE_CSS = """<style>
/* ── 모바일 타입 스케일 하한(F) — 데스크톱 밀도용 9~12px 마이크로 타이포를 모바일에서 일괄 상향.
   !important 로 인라인 스타일까지 교정. 데스크톱(>768px) 무변화. 자동 생성 후 수기 검수. ── */
@media(max-width:768px){
.agr-therm-scale,
.aj-auto,
.aj-chart-hint,
...
.pd-day span,
.pd-header .bk-badge,
.pd-holding-name span,
.pd-mini-legend,
.pd-row-main .hold-logo,
.pd-row-name span,
.pd-row-num small,
.pd-stat span,
...
.rsk-myr-sub,
.rsk-sev-badge,
.sv-empty .es-chip{font-size:12px!important}
.agr-comp .note,
.agr-foot,
.agr-interp,
.agr-title,
...
.pd-cat-head span,
.pd-conc-label,
.pd-diagnosis-head span,
.pd-diagnosis-item span,
.pd-hero small,
.pd-hero span,
.pd-insight summary,
.pd-insight-body,
...
.pd-overview-note,
.pd-risk-item span,
.pd-row-actions summary .pd-caret,
.pd-row-detail,
.pd-row-num,
...
.sh-s,
.sv-empty .es-sub,
.sv-glossary .gl-item span,
.todo-sev,
.todo-title{font-size:12.5px!important}
.agr-band .pill,
.agr-comp,
.aj-headline .lbl,
.at-empty,
...
.pd-alloc-name,
.pd-day b,
.pd-diagnosis-item strong,
.pd-empty p,
.pd-header p,
.pd-rebalance-card p,
.pd-risk-item b,
.pd-row-name b,
.port-detail-head span,
.port-map-head span,
.rb-trow,
.rbm-diag,
.rbm-pending-p,
...
.sv-acct-name,
.sv-empty .es-msg,
.sv-glossary .gl-item b,
.sv-nav a,
.todo-act,
.todo-expo{font-size:13.5px!important}
}
</style>"""
...
def inject_css():
    import re
    combined = GLOBAL_CSS + _METRIC_CSS + _SH_CSS + _FIN_CSS + _MARKET_CSS + _JEJU_CSS + _MOBILE_TYPE_CSS
